...
- **Closed but for one word, 14 Sep 2026.** The board's printed Quick Installation Guide (in the box, both sides) draws all three power connectors with pinouts — `ATXPWR1` **24-pin**, `ATX12V1` **8-pin**, `ATX12V2` **4-pin** — so the counts are now the board's own. What it never states in words is whether the 4-pin is mandatory; it shows it as part of power installation, both get populated, and this was never a purchase risk. The same read settled the power button, the fan headers, the POST-code display, the `PANEL1` pinout, the DIMM clip and the M.2 nut positions, and turned up one thing nobody had: **`M2_1` can be switched off by the `PE16_SEL` jumper, so the system SSD goes in `M2_2`.** See `notes/board-manual.md`.
...
2. **The build, step by step** — twelve steps, each with: *what you're doing · what it should look like/feel like · stop if …*. Jargon boxes inline the first time a term appears (POST, BMC/IPMI, RDIMM/ECC, EPS/ATX12V, 1DPC, M.2, UEFI, PWM, memory training).
   - **0 Before opening anything.** What "open bench" means and why it's normal. Static: touch the PSU shell or a grounded metal object first, work on the board's box, keep parts in their bags until used. Read the ROMED8-2T manual's layout page once and find on the real board: socket SP3, DIMM slots A1–H1, both M.2 sockets (M2_2 is the one used), ATXPWR1 (24-pin), ATX12V1 (8-pin), ATX12V2 (4-pin), `PANEL1`, the seven `FAN1`–`FAN7` headers, Dr. Debug, the dedicated `IPMI_LAN1` port, `VGA1`, LAN1/LAN2.
   - **1 CPU into the SP3 socket.** Verified procedure: T20 Torx; three captive screws, lettered A/B/C on ASRock's sheet; **the board's own guide says A→B→C to open and to close**, where AMD's SP3 documentation says loosen in reverse — both agree on the fastening order, which is the one that seats it; torque 1.58 ± 0.1 N·m (AMD). An **internal plastic cover** comes out of the rail frame first. Open the force frame, lift the rail frame, the CPU stays in its blue/orange **carrier frame** — slide the carrier into the rail frame until it clicks, orientation triangle to the socket's triangle, lower rail frame, lower force frame, fasten A→B→C. Never touch the socket pins; never remove the CPU from its carrier. Stop if a screw resists before it bottoms out.
   - **2 Memory.** All eight sticks, one per slot, all eight identical (the kit). 1 DIMM per channel means every slot is used, so no population-order decision. Notch is off-centre — match it before pushing; press both ends evenly, then **lock the single clip** by hand — these slots have one clip, not two, and ASRock makes locking it a step of its own; a stick that needs force is misaligned. Explain ECC RDIMM in one line. Photograph the labels first.
   - **3 NVMe.** **M2_2** (M2_1 can be switched off by the PE16_SEL jumper), 2280 length, single-sided drive: standoff at nut position **D**, 8cm — peel the yellow film off the nut first — insert at ~20°, press flat, one screw snug, not overtightened. The board has no SATA; that's why the G-DRIVE stays external.
   - **4 Cooler.** ARCTIC's SP3/TR4 path: bracket screws come up from under the mounting frame into the socket's own holes; PH1 required. Paste: the SP3 heat spreader is huge — spread the MX-6 thin over the whole lid with the included spreader or a card; 0.8g is one application, so no second try without more paste. Orientation: fan airflow arrow pointing at the rear I/O panel. Clip both fans; plug the fan lead(s) into `FAN1` — this board has no processor fan header, only `FAN1`–`FAN7` system fan connectors (confirm in the box whether the two fans share one PWM cable).
   ...
   - **Troubleshooting** table: no BMC on the network (wait 2 min, check the IPMI port not LAN1, check the router leases); no video (BMC VGA only works once the BMC is up); fans at full speed (normal before the BMC takes control); a Dr. Debug two-digit code (look it up; no speaker was bought, so there are no beeps); memory shows 64GB (sticks not seated — check every clip); the NVMe missing from the BIOS (it is in M2_1, which a jumper can disable).

**Facts verified, cited in the guide's footer.** Superseded in part on 14 Sep 2026 by the board's own printed Quick Installation Guide — see `notes/board-manual.md`, which is now the primary source for everything board-specific. SP3: T20, 1.58 ± 0.1 N·m (WikiChip, AMD community). ROMED8-2T: AST2500 BMC with VGA D-Sub, dedicated IPMI LAN, default `admin`/`admin` with forced change, BMC IP via BIOS Server Mgmt; power connectors ATXPWR1 24-pin, ATX12V1 8-pin, ATX12V2 4-pin. ARCTIC 4U-M Rev. 2: MX-6 0.8g in the box, PH1 required, two clip-on 120mm PWM fans, SP3 supported. MSI A850G: 24-pin, 2× EPS 4+4, 12V-2x6, 3× PCIe, 8 SATA, 4 Molex, fully modular. Kingston NV3: single-sided 2280, DRAM-less. **All since answered from the printed sheet, 14 Sep 2026** — no onboard power button, `PANEL1` pinout (`PWRBTN#` + `GND`), seven system fan headers and no CPU fan header, and a Dr. Debug display in place of beep codes. Only the Dr. Debug code table still needs the full User's Manual.
...
Nine drawn figures (inline SVG, same style as the Systems Map; no photos available from here): board map, rear ports, socket screws A→B→C and the layer stack, memory notch and clip, M.2 angle, paste spread and fan direction, power cabling, first-power timeline, and the `PANEL1` pinout. The board map and rear ports were redrawn from the printed sheet on 14 Sep 2026; anything not read from it is captioned as approximate. Render once, checker, republish, commit, push.
